...
<template>
  <div style="max-width: 640px;">
    <div class="dsker_section-header">
      <h3 class="dsker_section-title">技能</h3>
      <p class="dsker_section-desc">管理可用的技能模块。</p>
    </div>
    ...
    >
      <div style="display:flex; align-items:center; gap:12px;">
        <div style="flex:1; min-width:0;">
          <div style="font-size:14px; font-weight:500; margin-bottom:2px;">{{ s.name }}</div>
          <div style="font-size:12px; color:var(--color-text-tertiary);">{{ s.description }}</div>
        </div>
        <a-switch :checked="s.enabled" size="small" />
      </div>
    </a-card>
  </div>
</template>
